...
KalqSidePanel::KalqSidePanel(QWidget *parent) : QWidget(parent) {
  setupUi();
  connect(btn_search_, &QPushButton::clicked, this,
          &KalqSidePanel::onSearchClicked);

  connect(search_result_, &QTreeWidget::itemDoubleClicked, this,
          &KalqSidePanel::onItemDoubleClicked);
}
...
void KalqSidePanel::clearSearchResults() { search_result_->clear(); }

void KalqSidePanel::addSearchResult(const QString &foundString,
                                    const QString &sheet, const QString &cell,
...
void KalqSidePanel::setupUi() {
  ...
  search_result_->setHeaderLabels(
      {tr("Found String"), tr("Sheet"), tr("Cell")});
  search_result_->header()->setSectionResizeMode(0, QHeaderView::Stretch);
  search_result_->header()->setSectionResizeMode(1,
                                                 QHeaderView::ResizeToContents);
  search_result_->header()->setSectionResizeMode(2,
                                                 QHeaderView::ResizeToContents);
