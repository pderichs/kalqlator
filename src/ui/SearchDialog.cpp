...
void SearchDialog::setupUi() {
  ...
  m_scopeCombo->addItem(tr("Content and Formula"),
                        static_cast<int>(SearchScope::ContentAndFormula));
  m_scopeCombo->addItem(tr("Only Content"),
                        static_cast<int>(SearchScope::OnlyContent));
  m_scopeCombo->addItem(tr("Only Formula"),
                        static_cast<int>(SearchScope::OnlyFormula));
...
void SearchDialog::setupConnections() {
  connect(m_useRegexCheck, &QCheckBox::toggled, this,
          &SearchDialog::onUseRegexToggled);

  connect(m_clearButton, &QPushButton::clicked, this,
          &SearchDialog::onClearClicked);
...
void SearchDialog::applyOptions(const SearchOptions &options) {
  ...
  // Apply regex first so the toggled signal correctly enables/disables
  // caseSensitive
  m_useRegexCheck->setChecked(options.useRegularExpression);
  m_caseSensitiveCheck->setChecked(!options.useRegularExpression &&
                                   options.caseSensitive);
}

void SearchDialog::onClearClicked() {
  applyOptions(SearchOptions{.searchString = {},
