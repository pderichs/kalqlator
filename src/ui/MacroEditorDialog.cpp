...
MacroEditorDialog::MacroEditorDialog(MacroMap *macros, QWidget *parent)
  ...
  connect(m_triggerCombo, &QComboBox::currentTextChanged, this,
          &MacroEditorDialog::onTriggerChanged);
}
...
void MacroEditorDialog::accept() {
  ...
  try {
    ...
    EventDispatcher::dispatch(MacroEditorErrorEvent{
        .macro = m_previousTrigger, .def = code, .message = e.what()});
...
void MacroEditorDialog::setupUi() {
  ...
  if (m_macros->contains(Trigger_OnLoad)) {
    m_editor->setPlainText(
        QString::fromStdString(m_macros->at(Trigger_OnLoad)));
  }
...
void MacroEditorDialog::onTriggerChanged(const QString &trigger) {
  ...
  m_macros->insert_or_assign(m_previousTrigger,
                             m_editor->toPlainText().toStdString());
