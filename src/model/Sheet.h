...
class Sheet {
public:
  explicit Sheet(std::string identifier, std::string name,
   ...
   */
  void update_cell(int row, int column, const std::string &cell_name,
  ...
  [[nodiscard]] const LocationSet &get_selected_cells() const {
    return selected_cells_;
  }

  void set_selected_cells(const LocationSet &current_selection) {
    selected_cells_ = current_selection;
  }

  [[nodiscard]] const Location &get_current_selected_cell() const {
    return current_selected_cell_;
  }
  ...
  const std::unordered_map<size_t, size_t> &get_column_widths() const {
    return column_widths_;
  }
  const std::unordered_map<size_t, size_t> &get_row_heights() const {
    return row_heights_;
  }
  ...
  static bool matches_search(const SearchOptions &options, const Cell *cell,
                             std::string *out_complete_match);
