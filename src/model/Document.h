...
public:
  ...
  template <typename Predicate> Sheet *find_sheet(Predicate pred) const;
  ...
  [[nodiscard]] size_t sheet_count() const { return sheets_.size(); }
  ...
  void select_sheet_and_cell(const std::string &table_name,
...
private:
  ...
  MacroMap macros_;
  std::unordered_map<std::string, lisp::LispObjectPtrVector>
      parsed_macro_cache_;
};
