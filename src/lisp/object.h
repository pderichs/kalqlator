 ...
 */
using NativeFn =
    std::function<LispObjectPtr(const LispObjectPtr &, const std::any &)>;
...
using LispValue =
    std::variant<std::monostate, std::string, NumberRepresentation, True, Cons,
                 Symbol, NativeFn, LambdaFunction, UserFn, Nil>;

struct LispObject {
  LispValue data;

  [[nodiscard]] bool is_number() const {
    return std::holds_alternative<NumberRepresentation>(data);
  }
  ...
  [[nodiscard]] bool is_lambda() const {
    return std::holds_alternative<LambdaFunction>(data);
  }
  ...
  [[nodiscard]] bool is_true() const {
    return std::holds_alternative<True>(data);
  }
  ...
  [[nodiscard]] mpq_class as_number() const {
    return mpq_class_from_decimal_or_int(
        std::get<NumberRepresentation>(data).content);
  }
  ...
  [[nodiscard]] Symbol as_symbol() const { return std::get<Symbol>(data); }

  [[nodiscard]] Cons as_cons() const { return std::get<Cons>(data); }

  [[nodiscard]] bool is_false() const { return is_nil(); }

  [[nodiscard]] bool is_native_fn() const {
    return std::holds_alternative<NativeFn>(data);
  }

  [[nodiscard]] NativeFn as_native_fn() { return std::get<NativeFn>(data); }

  [[nodiscard]] std::string as_symbol_name() const { return as_symbol().name; }

  [[nodiscard]] bool is_truthy() const { return !is_nil(); }
};

inline void debug_print(const LispObjectPtr &obj,
