...
void LispQuoteTests::setq_lambda() {
  LispObjectPtrVector lisp = parse_all_string(
      R"((setq my-func (lambda (x y) (eql x y))) (my-func "a" "a"))");
...
void LispQuoteTests::setq_int() {
  LispObjectPtrVector lisp =
      parse_all_string("(setq answer-to-everything 42) (+ answer-to-everything "
                       "answer-to-everything answer-to-everything)");
  EnvironmentPtr env = std::make_shared<DefaultEnvironment>();
  Evaluator evaluator(env, {});
  auto result = evaluator.evaluate(lisp);
...
void LispQuoteTests::setq_string() {
  LispObjectPtrVector lisp =
      parse_all_string("(setq greeting \"Hello, World!\") greeting");
  EnvironmentPtr env = std::make_shared<DefaultEnvironment>();
  Evaluator evaluator(env, {});
  auto result = evaluator.evaluate(lisp);
...
void LispQuoteTests::setq_list() {
  LispObjectPtrVector lisp =
      parse_all_string("(setq my-list '(1 2 3)) (car my-list)");
  EnvironmentPtr env = std::make_shared<DefaultEnvironment>();
  Evaluator evaluator(env, {});
  auto result = evaluator.evaluate(lisp);
  QVERIFY(result->is_number());
...
void LispQuoteTests::setq_nested_reference() {
  LispObjectPtrVector lisp =
      parse_all_string("(setq a 5) (setq b (* a a)) (setq c (+ a b)) c");
  EnvironmentPtr env = std::make_shared<DefaultEnvironment>();
  Evaluator evaluator(env, {});
  auto result = evaluator.evaluate(lisp);
  QVERIFY(result->is_number());
...
void LispQuoteTests::setq_symbol_reference() {
  ...
  LispObjectPtrVector lisp = parse_all_string(
      "(setq original 42) (setq copy original) (setq original 100) copy");
  EnvironmentPtr env = std::make_shared<DefaultEnvironment>();
  Evaluator evaluator(env, {});
  auto result = evaluator.evaluate(lisp);
  QVERIFY(result->is_number());
  QCOMPARE(result->as_number(),
           mpq_class(42)); // copy sollte den Wert behalten, nicht die Referenz
}
