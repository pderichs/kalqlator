...
using CheckValue = std::variant<ExpectedType, std::string, Symbol, Cons, Nil,
                                const char *, mpq_class>;
 ...
 */
inline void checkCons(const LispObjectPtr &obj,
                      std::initializer_list<CheckValue> expected) {
  QVERIFY(obj->is_cons());
